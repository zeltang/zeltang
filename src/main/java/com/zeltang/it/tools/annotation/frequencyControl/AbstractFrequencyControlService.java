...
import com.zeltang.it.tools.utils.AssertUtil;
import com.zeltang.it.tools.utils.CommonErrorEnum;
 ...
 */
public abstract class AbstractFrequencyControlService<K extends FrequencyControlVo> {
    @PostConstruct
    ...
    public <T> T executeWithFrequencyControlList(List<K> frequencyControlList, SupplierThrowWithoutParam<T> supplier) throws Throwable {
        ...
        Map<String, FrequencyControlVo> frequencyControlDTOMap = frequencyControlList.stream().collect(Collectors.groupingBy(FrequencyControlVo::getKey, Collectors.collectingAndThen(Collectors.toList(), list -> list.get(0))));
